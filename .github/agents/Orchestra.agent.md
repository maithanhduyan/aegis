...
handoffs:
  - label: Khởi động thảo luận
    ...
    prompt: Tạo đề tài thảo luận với nội dung trên.
    send: true
...
- **GPT-Visionary-Agent (Visionary)** — Tầm nhìn xa 5-10-20 năm, chiến lược dài hạn
- **Gemini-Pragmatist-Agent (Pragmatist)** — Thực tế, khả thi, ROI ngắn hạn
...
2. Nghiên cứu context của dự án bằng cách đọc các file liên quan trong workspace (đặc biệt `docs/`, `src/`, `tests/`)
...
- Gọi **GPT-Visionary-Agent** với đề bài + context → GPT tạo file `review_gpt_round{N}_{YYYY-MM-DD}.md`
- Gọi **Gemini-Pragmatist-Agent** với đề bài + context → Gemini tạo file `review_gemini_round{N}_{YYYY-MM-DD}.md`
